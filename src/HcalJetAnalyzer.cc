...
#include "RecoHcal/HcalPromptAnalysis/interface/HcalJetAnalyzer.h"
...
//	 iEvent.getByType(hbhe);
	...
	int    IS=0;
	   ...
	   // if(eta<-16 || eta>16) continue;
	    ...
	    int p1=Phi0+1,p2=Phi1+1;
	    PHI=55; 
	    if(p1==p2 && p1==PHI){
	       if(phi>=(PHI-3) && phi<=(PHI+3) && z0>-180 && z1<0){
	         IS=1;
		 for(int k=0;k<10;k++)
		      DATA[phi][-eta][k]=adc2fC[digi.sample(k).adc()]-calibs.pedestal(digi.sample(k).capid());
	       }
	    }
     ...
     for(size_t j=0;j<numRecHits;j++) {
      ...
      if( DetNum == DetId::Hcal ){ 
	    ...
	    if(  HcalNum == HcalBarrel ){
	      ...
	      //opt << "         RecHit: " << j << ": HB, ieta=" << HcalID.ieta() << ", iphi=" << HcalID.iphi()<<      
